...
type OfdOrder = {
  ...
  outForDeliveryAt: string; deliveredAt: string; previousUndelivered: boolean;
};
...
export default function AnalyticsPanel({ mode, active }: { mode: "overview" | "today_ofd"; active: boolean }) {
  ...
  if (mode === "today_ofd") {
    ...
    return <section className={`analytics-view ${!active ? "view-hidden" : ""}`}>
      ...
      <section className="analytics-card ofd-orders-card">
        <header><div><h2>Today’s OFD orders</h2><p>Live status after going out for delivery on {ofdDate}.</p></div><span className="record-count">{ofd?.orders.length || 0} orders</span></header>
        <div className="analytics-table-wrap"><table className="analytics-table"><thead><tr><th>Order</th><th>Customer</th><th>OFD time</th><th>Current outcome</th><th>AWB / Courier</th><th>Amount</th></tr></thead><tbody>
          {(ofd?.orders || []).map((order) => <tr key={order.id}><td><strong>#{order.channelOrderId || order.id}</strong>{order.previousUndelivered && <small className="repeat-attempt">Repeat attempt</small>}</td><td><strong>{order.customerName || "—"}</strong><small>{[order.customerCity, order.customerState].filter(Boolean).join(", ")}</small></td><td>{formatDateTime(order.outForDeliveryAt)}</td><td><span className="analytics-status">{order.status || "Unknown"}</span></td><td><strong>{order.awb || "—"}</strong><small>{order.courier || "Not assigned"}</small></td><td><strong>{formatCurrency(order.total)}</strong></td></tr>)}
  ...
  const delivered = metrics.delivered?.percent || 0;
  const rto = metrics.rto?.percent || 0;
  const ndr = metrics.ndr?.percent || 0;
  ...
  return <section className={`analytics-view ${!active ? "view-hidden" : ""}`}>
    ...
    <div className="metrics-grid">
      <MetricCard label="Prepaid" metric={metrics.prepaid} /> <MetricCard label="Delivery success" metric={metrics.deliveryRate} hint="Of final delivery outcomes" />
      <MetricCard label="RTO" metric={metrics.rto} /> <MetricCard label="NDR / undelivered" metric={metrics.ndr} />
      <MetricCard label="Non-shipped" metric={metrics.nonShipped} /> <MetricCard label="Cancelled" metric={metrics.cancelled} />
    </div>
    <div className="analytics-grid overview-grid">
      <section className="analytics-card outcome-card"><header><div><h2>Order outcome mix</h2><p>Share of all filtered orders</p></div></header><div className="donut-layout"><div className="donut" style={{ background: `conic-gradient(#46d495 0 ${delivered}%, #ff706b ${delivered}% ${delivered + rto}%, #f0aa5c ${delivered + rto}% ${delivered + rto + ndr}%, #27302c ${delivered + rto + ndr}% 100%)` }}><span><strong>{metrics.total?.count || 0}</strong><small>orders</small></span></div><div className="legend"><span><i className="delivered" />Delivered <b>{delivered}%</b></span><span><i className="rto" />RTO <b>{rto}%</b></span><span><i className="ndr" />NDR <b>{ndr}%</b></span><span><i />Other <b>{Math.round(other * 10) / 10}%</b></span></div></div></section>
