    ...
    <section className={`confirmation-view ${active ? "" : "view-hidden"}`}>
      ...
      {!loading && section === "campaigns" && <div className="campaign-layout"><article className="confirmation-card campaign-panel">
        ...
        <div className="campaign-list">{data.campaigns.map((campaign, index) => <div className={`campaign-row ${campaign.isActive ? "" : "inactive"} ${draggedCampaignId === campaign.id ? "dragging" : ""}`} key={campaign.id} onDragOver={(event) => event.preventDefault()} onDrop={() => void dropCampaign(campaign.id)}>
          ...
          <div className="campaign-more-wrap"><button className="campaign-more" type="button" aria-label={`Actions for ${campaign.name}`} onClick={() => setOpenCampaignMenu((current) => current === campaign.id ? "" : campaign.id)}><MoreHorizontal size={18}/></button>{openCampaignMenu === campaign.id && <div className="campaign-menu">{campaign.id === "cmp_default_high_rto" || !campaign.isActive ? <span>{campaign.id === "cmp_default_high_rto" ? "Permanent campaign" : "Campaign inactive"}</span> : <button disabled={busy} type="button" onClick={() => { setOpenCampaignMenu(""); void post({ action: "deactivate_campaign", campaignId: campaign.id }); }}>Deactivate</button>}</div>}</div>
        </div>)}</div>
      </article></div>}
