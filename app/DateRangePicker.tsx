...
export default function DateRangePicker({from,to,max,onApply}:{from:string;to:string;max:string;onApply:(from:string,to:string)=>void}) {
  ...
  return <div className="date-range-control"><span>Order date range</span><button type="button" onClick={()=>{setStart(from);setEnd(to);setMonth(new Date((from||max)+"T12:00:00"));setOpen(true);}}><CalendarDays size={16}/>{from && to ? `${from} — ${to}` : "All history"}</button>
