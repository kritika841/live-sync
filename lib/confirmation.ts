...
function orderTags(raw: Record<string, unknown>) {
  const values = [raw.order_tag, raw.sr_tags].flatMap((value) => Array.isArray(value) ? value : String(value ?? "").split(","));
...
function campaignMatches(order: RoutingOrder, criteria: CampaignCriteria) {
  ...
  if (criteria.paymentMethod && criteria.paymentMethod !== "ANY" && normalized(order.paymentMethod) !== normalized(criteria.paymentMethod)) return false;
  if (criteria.tags?.length) {
    const tags = orderTags(raw);
...
export async function routeConfirmationOrders(db: PostgresDatabase, orderIds: number[]) {
  ...
  const [campaignResult, orderResult] = await Promise.all([
    ...
    db.prepare(`SELECT id, payment_method AS paymentMethod, products_json AS productsJson, raw_json AS rawJson, status,
