...
if(periods.some(p=>!p.metricMatch||p.missing.length||p.mismatches.length||p.dateMismatches.length||p.extra.length)) {
 console.error('Provider reconciliation found discrepancies; inspect shiprocket_validation_json in sync_state');
 if(process.env.FAIL_SHIPROCKET_VALIDATION==='true')process.exitCode=1;
} else console.log('Shiprocket reconciliation passed:',JSON.stringify(summary));
